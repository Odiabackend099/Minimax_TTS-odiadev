#!/usr/bin/env python3
"""Generate CallWaiting AI demo using local MiniMax client as fallback."""
...
def test_render_service():
    ...
    try:
        import requests
        response = requests.get("https://minimax-tts-odiadev.onrender.com/health", timeout=10)
        if response.status_code == 200:
            print("✅ Render service is healthy")
            return True
        else:
            print(f"❌ Render service health check failed: {response.status_code}")
            return False
    except Exception as e:
        print(f"❌ Render service not accessible: {e}")
        return False

def generate_with_local_client():
    """Generate TTS using local MiniMax client."""
    print("🎤 Generating TTS using local MiniMax client...")
    ...
    try:
        ...
        if 'audio_base64' in result:
            ...
            filename = f"callwaiting_marcus_local_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp3"
...
def generate_all_voices():
    """Generate TTS with all verified voices."""
    print("\n🎭 Generating TTS with all verified voices...")
...
def main():
    """Main function."""
    print("🎯 CallWaiting AI - Marcus Voice Demo (Fallback)")
    print("=" * 60)
    print("Generating CallWaiting AI script using verified voice IDs...")
    print(f"Script: {CALLWAITING_SCRIPT[:100]}...")
    print()
    
    # Test Render service
    render_healthy = test_render_service()
    
    # Generate CallWaiting demo with Marcus voice
    print(f"\n🎤 Generating CallWaiting AI demo...")
    if generate_with_local_client():
        print("🎉 SUCCESS! CallWaiting AI demo generated with Marcus voice!")
    else:
        print("❌ Failed to generate CallWaiting AI demo")
    
    # Generate all voices
    successful_voices = generate_all_voices()
    
    print(f"\n📊 FINAL RESULTS")
    print("=" * 60)
    print(f"✅ Render service healthy: {render_healthy}")
    ...
    if successful_voices > 0:
        print(f"\n🎉 ODIADEV AI TTS voices are working!")
        print(f"All verified voice IDs are generating audio successfully!")
        print(f"Service URL: https://minimax-tts-odiadev.onrender.com")
    else:
        print(f"\n❌ No successful voice generations")
